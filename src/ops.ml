...
let unwrap_vec (o : ovalue) : float list =
    ...
    match o with
    | ArrValue a -> List.fold_right 
        (fun x -> List.cons (match x with | CoreValue Float f -> f 
            | CoreValue Num n -> float_of_int n | _ -> fail ())) a []
    | _ -> fail()
...
and internal_fn (name : id) (args : ovalue list) : ovalue =
    let fail () = failwith 
        ("Invalid application of " ^ name ^ " with arguments " ^ string_of_list string_of_ovalue args) in
    let as_unop () : ovalue = match args with | [a] -> a | _ -> fail () in
    let as_binop () : ovalue * ovalue = match args with | [a;b] -> a,b | _ -> fail () in
...
and eval_exp (e : exp) (t : etyp) (fns : fn list) (s : sigma) : ovalue * sigma =
    ...
    | Arr a ->
        let result,s' = List.fold_left 
        (fun (vl, s_g) e -> let v, s = eval_texp e fns s in vl@[v], s) ([], s) a in
        ArrValue result, s'
    | Index _ -> failwith "unimplemented op"
    | FnInv (id, tl, args) -> let (fn, p) = fn_lookup id fns in
        let arg_vs, s' = (List.fold_left (fun (vl, s) e -> 
...
and eval_comm (c : comm) (fns : fn list) (s : sigma) : sigma =
    ...
    | If ((b, c1), el, c2) ->
        ...
        let check_cmd b c acc = (match acc with
            | Some s -> acc
            | None -> let v, s' = check_if b s in
                if v then let _, s' = eval_cl c fns s in Some s'
                else None)
        in
        (match List.fold_left (fun acc (b, c) -> check_cmd b c acc) (check_cmd b c1 None) el with
        | Some s' -> s'
        | None -> (match c2 with | Some c -> (let _, s' = eval_cl c fns s in s') | None -> s))
    | For (c1, b, c2, cl) ->
        let rec loop s = (match (eval_texp b fns s) with
...
and eval_cl (cl : comm list) (fns : fn list) (s : sigma) : ovalue * sigma =
    ...
    | [] -> CoreValue Unit, s
    | h::t -> (match h with
        | Return Some e -> let v, s_g = eval_texp e fns s in v, s
