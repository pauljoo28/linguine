...
let is_valid (cx : contexts) (v : string) : unit =
  if Assoc.mem v cx._bindings.rv then
    ...
    | INVALID -> error cx (v ^ " has not been updated yet")
    | VALID -> ()
  else
    ()

let rec helper_validate_var (cx : contexts) (spec_lst : string list) (valids : valid context) : unit =
  match spec_lst with
  | [] -> ()
  | h :: t ->
    if Assoc.mem h valids then
      match Assoc.lookup h valids with
      | INVALID -> error cx (h ^ " has not been updated yet")
      | VALID -> helper_validate_var cx t valids
    else
      helper_validate_var cx t valids
...
let validate_var (cx : contexts) (e : aexp) : contexts * aexp =
  ...
  | Var v ->
      ...
      helper_validate_var cx spec_lst cx._bindings.rv;
      CheckUtil.bind cx v (Val (VALID)), get_stip cx v
  | _ -> error cx "Update expression can only be used with a variable"
...
and invalidate_var (cx : contexts) (e : aexp) : contexts =
  ...
  | _ -> error cx "Invalidate expression can only be used with a variable"
